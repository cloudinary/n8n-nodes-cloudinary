...
3. **Aspect Ratio + the default Smart crop can't merge with a source transformation.** Setting `aspectRatio` makes the player apply *its own* crop (controlled by `cropMode`, exposed as the **Crop Mode** field — `smart`/`fill`/`pad`, default `smart`). Per the player docs, when both a player transformation and a source `transformation` exist, "the transformation definitions get merged." The default `smart` crop contributes an auto-gravity component, and merging it into the user's `raw_transformation` places it wrongly — the player rejects it with "g_auto must be in a transformation component by itself". `fill`/`pad` crop without auto-gravity, so they merge cleanly. The handler fails fast on the exact breaking combination — `aspectRatio` set **and** `cropMode === 'smart'` (the default) **and** a non-empty `transformation` — pointing the user at the two fixes (pick Fill/Pad, or clear Aspect Ratio). The Crop Mode field's option/help text avoids Cloudinary qualifier jargon (`g_auto`) by deliberate request — it's framed in shape terms ("crops it a second time") for non-technical users; keep that framing if you edit it. The `source[cropMode]` param is emitted only when an aspect ratio is set and the mode is non-default, mirroring how `skin`/`autoplayMode` stay out of clean URLs.
