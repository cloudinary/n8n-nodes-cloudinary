 ...
 * or a specific timestamp (`so_<t>`). Like the other transform ops, an optional
 * `Continue From Transformation` is prepended before the frame selector (handled in
 * buildTransformResult), so callers can chain from a previous transform step (e.g.
 * pipe `{{ $json.transformation }}` from a Trim or Multi-Step node).
 */
