...
- `npm run build` — compile TypeScript and copy SVG/PNG icons into `dist/` via gulp. The `n8n` block in `package.json` points at compiled paths in `dist/`, so n8n only loads the build output.
...
- [nodes/Cloudinary/cloudinary.utils.ts](nodes/Cloudinary/cloudinary.utils.ts) — shared helpers used by the handlers: `generateCloudinarySignature` (Cloudinary's signed-upload scheme: sorted params, exclude `file`/`api_key`/`signature`, SHA-256 with api_secret appended), `createMultipartBody` (hand-rolled multipart for binary file uploads — no form-data dependency), the URL builders (`buildUploadUrl`, `buildResourceUpdateUrl`), `metadataToPipeString`, `buildSearchExpression`, and `extractCloudinaryError`.
...
n8n serializes each saved workflow as JSON keyed by **parameter `name`**. A workflow stores only the values the user set; on load, any parameter absent from the JSON falls back to the field's `default`. This has hard consequences when evolving the node:

- **Never rename or remove a parameter `name` that shipped.** Doing so orphans the stored value in existing workflows; required fields then render empty/invalid. Renaming `displayName` is safe (UI only); renaming `name` is a breaking change.
- **Never repurpose a field's meaning** (e.g. reuse `type` for something else). It silently changes behavior in saved workflows with no error.
- **Changing a pure util signature/output is not itself the break** — the break is the parameter-schema change that feeds it. Treat utils additively too: add a new builder rather than overloading an existing one.

**To add a new variant of an existing operation (e.g. a second way to identify the asset), do it additively:**

1. Add a mode selector (`options`) whose **default value reproduces the current behavior**, so workflows that predate it (no stored value) keep working unchanged.
2. Gate the existing fields behind that default; gate the new fields behind the new mode. Leave existing field `name`s untouched.
3. Add a new helper for the new path instead of changing the existing one; branch in the handler on the mode.

Worked example — Update Asset can also target an asset by **asset ID** instead of `resourceType` + `type` + `publicId`. Note the endpoints differ in **method**, not just path:
- Public ID: `POST /resources/:resource_type/:type/:public_id` (existing `buildResourceUpdateUrl`).
- Asset ID: `PUT /resources/:asset_id` (would need a new `buildResourceUpdateByAssetIdUrl`).

The compatible implementation: add an "Identify By" selector defaulting to `Public ID`, gate the three existing fields behind it, add an `assetId` field behind the `Asset ID` mode, and have the handler choose URL + HTTP method from the selected mode.
